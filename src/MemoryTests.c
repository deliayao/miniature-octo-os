...
void results(void) {
    ...
    while (1) {
        ...
        if (!completed && get_process_priority(PROCESS_1) == LOWEST && get_process_priority(PROCESS_2) == LOWEST) {
            if (passed[0] == PASS) {
                uart0_put_string("Request memory: OK");
            } else {
                uart0_put_string("Request memory: FAILED");
            }
            uart0_put_string("\r\n");
            if (passed[1] == PASS) {
                uart0_put_string("Release memory: OK");
            } else {
                uart0_put_string("Release memory: FAILED");
            }
            uart0_put_string("\r\n");
            if (passed[2] == PASS) {
                uart0_put_string("Remaining memory: OK");
            } else {
                uart0_put_string("Remaining memory: FAILED");
            }
